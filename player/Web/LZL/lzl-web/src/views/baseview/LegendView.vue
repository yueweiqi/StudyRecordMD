<template>
  <el-dialog v-model="addForm.visible" :title="addForm.aOrUText">
     <el-form :model="addForm">
       <el-form-item label="英雄名称" :label-width="addForm.labelWidth">
         <el-input v-model="addFormData.name" autocomplete="off" />
       </el-form-item>
       <el-form-item v-show="false" label="英雄头像" :label-width="addForm.labelWidth">
 ...
 const onaddFromClick=()=>{
   if(addForm.addOrUpdate==1){
     http.post("/Legend/Add",addFormData).then((res)=>{
       console.log(res);
       ElMessage({
         showClose: true,
         message: '添加成功',
   ...
   if(addForm.addOrUpdate==2){
     http.post("/Legend/Update",addFormData).then((res)=>{
       console.log(res);
       ElMessage({
         showClose: true,
         message: '修改成功',
 ...
 const pageDataChange=()=>{
   http.post("/Legend/GetPageData",tablePage)
   .then((res)=>{
